...
public interface ProgramRepository extends JpaRepository<Program, Long>, JpaSpecificationExecutor<Program> {
    ...
    @Modifying
    @Transactional
    @Query("UPDATE Program p SET p.status = com.grantserve.grantserve1.enums.ProgramStatus.CLOSED WHERE p.programID = :programId AND p.status = com.grantserve.grantserve1.enums.ProgramStatus.ACTIVE")
    int updateProgramStatusToClosed(@Param("programId") Long programId);

    @Query("SELECT p FROM Program p WHERE p.startDate <= :now AND p.endDate >= :now AND p.status = com.grantserve.grantserve1.enums.ProgramStatus.ACTIVE")
